...
//const { uploadImageToImgur } = require('../Hanstz/imgur');





hans({ nomCom: "tagall", categorie: 'Group', reaction: "📯" }, async (dest, zk, commandeOptions) => {
  ...
  if (!verifGroupe) { repondre("⚠️ 😤this command is reserved for groups ❌"); return; }
  if (!arg || arg === ' ') {
  mess = 'Aucun Message'
  } else {
    mess = arg.join(' ')
...
hans({ nomCom: "link", categorie: 'Group', reaction: "✌️" }, async (dest, zk, commandeOptions) => {
...
hans({ nomCom: "promote", categorie: 'Group', reaction: "👨🏿‍💼" }, async (dest, zk, commandeOptions) => {
...
hans({ nomCom: "demote", categorie: 'Group', reaction: "👨🏿‍💼" }, async (dest, zk, commandeOptions) => {
  let { repondre, msgRepondu, infosGroupe, auteurMsgRepondu, verifGroupe, auteurMessage, superUser, idBot } = commandeOptions;
  let membresGroupe = verifGroupe ? await infosGroupe.participants : ""
  if (!verifGroupe) { return repondre("For groups only"); }
...
/** ***fin démettre****  **/
/** **retirer** */
hans({ nomCom: "remove", categorie: 'Group', reaction: "👨🏿‍💼" }, async (dest, zk, commandeOptions) => {
    ...
    if (autAdmin || superUser) {
      if (msgRepondu) {
        if (zkad) {
          if (membre) {
            if (admin == false) {
              const gifLink = "https://raw.githubusercontent.com/djalega8000/hans-MD/main/media/remover.gif"
...
hans({ nomCom: "del", categorie: 'Group',reaction:"🧹" }, async (dest, zk, commandeOptions) => {
...
hans({ nomCom: "info", categorie: 'Group' }, async (dest, zk, commandeOptions) => {
 ...
 hans({ nomCom: "antilink", categorie: 'Group', reaction: "🔗" }, async (dest, zk, commandeOptions) => {
 ...
 hans({ nomCom: "antibot", categorie: 'Group', reaction: "👾" }, async (dest, zk, commandeOptions) => {
...
hans({ nomCom: "group", categorie: 'Group' }, async (dest, zk, commandeOptions) => {
...
hans({ nomCom: "left", categorie: "Mods" }, async (dest, zk, commandeOptions) => {
...
hans({ nomCom: "gname", categorie: 'Group' }, async (dest, zk, commandeOptions) => {
...
hans({ nomCom: "gdesc", categorie: 'Group' }, async (dest, zk, commandeOptions) => {
...
hans({ nomCom: "gpp", categorie: 'Group' }, async (dest, zk, commandeOptions) => {
...
/////////////
hans({nomCom:"hidetag",categorie:'Group',reaction:"🎤"},async(dest,zk,commandeOptions)=>{
...
hans({ nomCom: "apk", reaction: "✨", categorie: "Recherche" }, async (dest, zk, commandeOptions) => {
...
hans({
  ...
  const {arg , repondre , verifAdmin , superUser , verifhansAdmin } = commandeOptions ;

  if (verifAdmin || superUser) {

    if(!verifhansAdmin){ repondre('You need administrative rights to perform this command') ; return ;}
